<x-layoutdsbd title="{{ $title }}" user="{{ $user['name'] }}" role="{{ $user['role'] }}">
    <div class="container mx-auto w-full">
        <div class="w-full dark:text-white flex gap-2 p-2 lg:p-0 scrollbar-thin scrollbar-thumb-slate-300 scrollbar-track-slate-100 dark:scrollbar-thumb-slate-300 dark:scrollbar-track-slate-500">
            @can('checkin')
                <a href="{{ route('showCheckIn') }}" class="px-5 sm:px-10 py-2 shadow-md text-center bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 dark:text-white rounded-md">CheckIn</a>
            @endcan
            @can('checkout')
                <a href="{{ route('showCheckOut') }}" class="px-5 sm:px-10 py-2 shadow-md text-center bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 dark:text-white rounded-md">CheckOut</a>
            @endcan
            {{-- <a href="#" class="px-5 sm:px-10 py-2 shadow-md text-center bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 dark:text-white rounded-md">Mantenece</a> --}}
        </div>
    </div>
                ...
                <tbody id="tableBody">
                    ...
                    <tr class="text-slate-800 dark:text-slate-200 border-b border-slate-200 dark:border-slate-700 rounded-md">
                        ...
                        <td class="py-3 px-1 whitespace-nowrap max-w-[20ch] truncate text-center">
                            <a href="{{ route('masterAsset', $asset->slug ) }}" class="py-1.5 px-4 text-center shadow-md bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 dark:text-white rounded-md">
                            Detail</a>
                        </td>
                    </tr>
                    @empty
