<x-layoutdsbd title="{{ $title }}" user="{{ $user['name'] }}" role="{{ $user['role'] }}">
            ...
            <div class="flex items-center space-x-2">
                ...
                @can('import-category')
                    <button id="importButton"
                        data-modal-target="ModalImportCategory" 
                        data-modal-toggle="ModalImportCategory"
            ...
            <table id="tabelData" class="table-auto w-full text-left">
                <thead>
                    <tr class="sticky top-0 text-slate-900 dark:text-slate-100 bg-slate-100 dark:bg-slate-900 border-b-2 border-slate-200 dark:border-slate-800 shadow-md">
                        ...
                        <th class="py-3 px-1 whitespace-nowrap">Nama Category</th>
                ...
                <tbody  id="tableBody">
                    ...
                    <tr class="text-slate-800 dark:text-slate-200 border-b border-slate-200 dark:border-slate-700 rounded-md">
                        ...
                        <td class="py-3 px-1 whitespace-nowrap max-w-[20ch] truncate">{{ $category->category_name }}</td>
                        <td class="py-3 px-1 whitespace-nowrap max-w-[20ch] truncate">{{ $category->description }}</td>
                        @canany(['edit-category', 'delete-category'])
                            <td class="py-2 px-1 whitespace-nowrap max-w-[20ch] truncate text-center">
                                @can('edit-category')
                                    <button
                                    data-modal-target="ModalCategory" 
                                    data-modal-toggle="ModalCategory"
                                ...
                                @can('delete-category')
                                    ...
                                    data-name="{{ $category->category_name }}"
                                    data-action="{{ route('category.delete.action', $category->id ) }}"
    ...
    @canany(['edit-category','add-category'])
        <!-- ModalAddEdit -->
        <div id="ModalCategory" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
            <div class="relative p-4 w-full max-w-md max-h-full">
                ...
                <div class="relative bg-white rounded-lg shadow-sm dark:bg-slate-800">
                    ...
                    <div class="flex items-center justify-between p-3 md:p-4 border-b rounded-t dark:border-slate-600 border-slate-200">
                        ...
                        <button type="button" class="text-slate-400 bg-transparent hover:bg-slate-200 hover:text-slate-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-slate-600 dark:hover:text-white" data-modal-toggle="ModalCategory">
                    ...
                    <form id="formCategory" class="p-3 md:p-4" action="" method="POST">
                        ...
                        <div class="grid gap-3 mb-3 grid-cols-2">
                            ...
                            <div class="col-span-2">
                                <label for="nameCategory" class="block mb-2 text-sm font-medium text-slate-900 dark:text-white">Category Name</label>
                                <input type="text" name="nameCategory" id="nameCategory" class="bg-slate-50 border border-slate-300 text-slate-900 text-sm rounded-lg focus:ring-slate-50 focus:border-slate-600 block w-full p-2.5 dark:bg-slate-700 dark:border-slate-600 dark:placeholder-slate-400 dark:text-white dark:focus:ring-slate-500 dark:focus:border-slate-500" placeholder="Enter category name">
                            </div>
                            <div class="col-span-2">
            ...
            <div class="relative p-4 w-full max-w-md max-h-full">
                <div class="relative bg-white rounded-lg shadow dark:bg-slate-800">
                    ...
                    <div class="p-4 md:p-5 text-center">
                        ...
                        <h3 class="mb-5 text-lg font-normal text-slate-500 dark:text-slate-400">Are you sure you want to delete this Category?</h3>
                        <form id="deleteForm" action="" method="POST">
                            ...
                            <div class="space-x-2">
                                ...
                                <button data-modal-hide="modalDelete" type="button" class="py-2.5 px-5 text-center text-sm font-medium  shadow-md bg-slate-100 hover:bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-600 dark:text-white rounded-md">No, cancel</button>
                            </div>
                        </form>
                    </div>
                </div>
    ...
    @can('import-location')
        <!-- ImportModal -->
        <div id="ModalImportCategory" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
            <div class="relative p-4 w-full max-w-md max-h-full">
                <!-- Modal content -->
                <div class="relative bg-white rounded-lg shadow dark:bg-slate-800 ">
                    <!-- Modal header -->
                    <div class="flex items-center justify-between p-3 md:p-4 border-b rounded-t dark:border-slate-600 border-slate-200">
                        ...
                        <button type="button" class="text-slate-400 bg-transparent hover:bg-slate-200 hover:text-slate-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-slate-600 dark:hover:text-white" data-modal-toggle="ModalImportCategory">
                            <svg class="w-3 h-3" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6"/>
                            </svg>
                            <span class="sr-only">Close modal</span>
                    ...
                    <!-- Modal body -->
                    <form id="importForm" action="{{ route('category.import.action') }}" class="p-3 md:p-4 space-y-4" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="">
...
<script>
    ...
    function validateForm() {
        ...
        else if (nameCategory.length > 60) errors.push('Name Category maksimal 60 karakter');
