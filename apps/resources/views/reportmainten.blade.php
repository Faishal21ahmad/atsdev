<x-layoutdsbd title="{{ $title }}" user="{{ $user['name'] }}" role="{{ $user['role'] }}">
    ...
    <div class="container mx-auto w-full mt-5 text-slate-900 dark:text-slate-100">
        <div class="w-3/4">
            <div class="flex gap-4">
                <p id="titlecodeasset" class="text-lg">{{ $ItemAsset->code_assets }}</p>
                <p id="titleLocation" class="text-lg">{{ $ItemAsset->location->location_name ?? 'location Unknown'  }}</p>
            </div>
    ...
    <div class="container mx-auto w-full">
        <form id="reportMainten" action="{{ route('mainten.report.action') }}" method="POST" class="flex flex-col gap-4" enctype="multipart/form-data">
            ...
            <label for="problemDetail" class="block text-lg font-medium text-slate-900 dark:text-white">Problem Details</label>
            ...
            <label for="reportType" class="block text-lg font-medium text-slate-900 dark:text-white">Report Type</label>
            <ul class="grid lg:w-2/4 gap-4 md:grid-cols-2">
                <li>
                    <input type="radio" id="Repair" name="reportType" value="Repair" class="hidden peer"/>
                    <label id="reportType" for="Repair" class="inline-flex items-center justify-between w-full p-3 text-slate-300 bg-slate-50 border border-slate-200 rounded-lg cursor-pointer peer-checked:bg-slate-200 peer-checked:text-slate-900  peer-checked:shadow-md
                ...
                <li>
                    ...
                    <label id="reportType" for="Maintenance" class="inline-flex items-center justify-between w-full p-3 text-slate-300 bg-slate-50 border border-slate-200 rounded-lg cursor-pointer peer-checked:bg-slate-200 peer-checked:text-slate-900  peer-checked:shadow-md
                    dark:peer-checked:bg-slate-800 dark:peer-checked:text-white dark:peer-checked:border-slate-800
                    hover:text-slate-600 hover:bg-slate-100 dark:text-slate-400 dark:bg-slate-700 dark:hover:bg-slate-700 dark:border-0">
                        <div class="block">
            ...
            <button type="submit" class="w-32 py-2 px-4 text-center shadow-md bg-slate-100 hover:bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-600 dark:text-white rounded-md">Report</button>
        </form>
    </div>
    ...
    <script>
        ...
        function validateForm() {
            ...
            const problemDetail = document.getElementById('problemDetail').value.trim();
            ...
            else if (problemDetail.length > 300) errors.push('Problem Detail minimal 300 karakter');
