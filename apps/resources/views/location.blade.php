<x-layoutdsbd title="{{ $title }}" user="{{ $user['name'] }}" role="{{ $user['role'] }}">
            ...
            <table id="tabelData" class="table-auto w-full text-left">
                <thead>
                    <tr class="sticky top-0 text-slate-900 dark:text-slate-100 bg-slate-100 dark:bg-slate-900 border-b-2 border-slate-200 dark:border-slate-700 shadow-md">
                        ...
                        @canany(['delete-location','edit-location'])
    ...
    @canany(['edit-location','add-location'])
    <!-- ModalAdd -->
        ...
        <div class="relative p-4 w-full max-w-md max-h-full">
            ...
            <div class="relative bg-white rounded-lg shadow-sm dark:bg-slate-800">
                ...
                <div class="flex items-center justify-between p-3 md:p-4 border-b rounded-t dark:border-slate-600 border-slate-200">
                    ...
                    <button type="button" class="text-slate-400 bg-transparent hover:bg-slate-200 hover:text-slate-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-slate-600 dark:hover:text-white" data-modal-toggle="ModalLocation">
                ...
                <form id="formLocation" class="p-3 md:p-4" action="" method="POST">
                    ...
                    <div class="grid gap-3 mb-3 grid-cols-2">
                        ...
                        <div class="col-span-2">
                            <label for="nameLocation" class="block mb-2 text-sm font-medium text-slate-900 dark:text-white">Name Location</label>
                            <input type="text" name="nameLocation" id="nameLocation" class="bg-slate-50 border border-slate-300 text-slate-900 text-sm rounded-lg focus:ring-slate-50 focus:border-slate-600 block w-full p-2.5 dark:bg-slate-700 dark:border-slate-600 dark:placeholder-slate-400 dark:text-white dark:focus:ring-slate-500 dark:focus:border-slate-500" placeholder="Name Location Items">
                        </div>
                        <div class="col-span-2">
                            <label for="description" class="block mb-2 text-sm font-medium text-slate-900 dark:text-white">Location Description</label>
                            <textarea id="description" name="description" rows="4" class="bg-slate-50 border border-slate-300 text-slate-900 text-sm rounded-lg focus:ring-slate-50 focus:border-slate-600 block w-full p-2.5 dark:bg-slate-700 dark:border-slate-600 dark:placeholder-slate-400 dark:text-white dark:focus:ring-slate-500 dark:focus:border-slate-500" placeholder="Write location description here"></textarea>                    
                        </div>
                    </div>
        ...
        <div class="relative p-4 w-full max-w-md max-h-full">
            <div class="relative bg-white rounded-lg shadow-sm dark:bg-slate-800">
                ...
                <div class="p-4 md:p-5 text-center">
                    ...
                    <h3 class="mb-5 text-lg font-normal text-slate-500 dark:text-slate-400">Are you sure you want to delete this Location ?</h3>
            ...
            <div class="relative p-4 w-full max-w-md max-h-full">
                ...
                <div class="relative bg-white rounded-lg shadow-sm dark:bg-slate-800">
                    ...
                    <div class="flex items-center justify-between p-3 md:p-4 border-b rounded-t dark:border-slate-600 border-slate-200">
                        ...
                        <button type="button" class="text-slate-400 bg-transparent hover:bg-slate-200 hover:text-slate-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-slate-600 dark:hover:text-white" data-modal-toggle="ModalImportLocation">
                            <svg class="w-3 h-3" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6"/>
                            </svg>
                            <span class="sr-only">Close modal</span>
                    ...
                    <!-- Modal body -->
                    <form id="importForm" action="{{ route('location.import.action') }}" class="p-3 -mt-4  md:p-4 space-y-4" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="">
    ...
    <script>
        ...
        function validateForm() {
            const nameLocation = document.getElementById('nameLocation').value.trim();
            const description = document.getElementById('description').value.trim();
            const errors = [];
            if (!nameLocation) errors.push('Name Location tidak boleh kosong');
            else if (nameLocation.length > 60) errors.push('Name Location maksimal 60 karakter');
