<x-layoutdsbd title="{{ $title }}" user="{{ $user['name'] }}" role="{{ $user['role'] }}">
    ...
    <div id="containerInfo" class="container mt-2 w-full  text-slate-900 dark:text-slate-100 grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div id="infoMainten" class="">
                ...
                <div class="flex gap-4 justify-between">
                    ...
                    <div class="text-slate-900 dark:text-slate-100 ">
                        <div class="flex gap-4">
                            ...
                            <p id="location" class="text-lg">{{ $dataMainten->itemasset->location->location_name ?? 'location Unknown' }}</p>
                        </div>
                        <p id="nameasset" class="text-3xl font-semibold">{{ $dataMainten->itemasset->masterAsset->asset_name }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="my-2">
                ...
                <p class="text-md">{{ \Carbon\Carbon::parse($dataMainten->created_at)->format('d / m / Y H:i') }}</p>
            </div>
            <div class="my-2">
            ...
            <div class="my-2">
                ...
                <p class="text-md">{{ $dataMainten->reportedBy->username }}</p>
            </div>
            <div class="w-full h-1 my-4 bg-slate-200 dark:bg-slate-800 rounded-md"></div>
        </div> 
        ...
        <div id="default-carousel" class="relative w-full hover:cursor-pointer  " data-carousel="slide">
            ...
            <button type="button" class="absolute top-0 start-0 z-30 flex items-center justify-center h-full px-4 cursor-pointer group focus:outline-none" data-carousel-prev>
                <span class="inline-flex items-center justify-center w-10 h-10 rounded-full bg-white/30 dark:bg-slate-800/30 group-hover:bg-white/50 dark:group-hover:bg-slate-800/60 group-focus:ring-4 group-focus:ring-white dark:group-focus:ring-slate-800/70 group-focus:outline-none">
                    <svg class="w-4 h-4 text-white dark:text-slate-800 rtl:rotate-180" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 6 10">
            ...
            @if ($imagesFile != null)
                <div class="group">
                    ...
                    <button data-modal-target="default-modal-11" data-modal-toggle="default-modal-11"
                        class="absolute z-30 inset-0 mx-auto w-[70%] opacity-0 group-hover:opacity-70 transition-opacity duration-300 text-white bg-slate-700 hover:bg-slate-800 font-medium rounded-lg text-sm text-center dark:bg-slate-600 dark:hover:bg-slate-700 "
                        type="button">
                        Open
                    </button>
                </div>
    ...
    <form id="formResolveMainten" action="{{ route('mainten.resolve.action') }}" method="post" enctype="multipart/form-data">
        ...
        <div class="mt-3">
            <label hidden for="repairDetail" class="block mb-2 mt-5 text-lg font-medium text-slate-900 dark:text-white">Repair Details</label>
        ...
        <div class="mt-3">
            <label for="vendor" class="block mb-2 text-lg font-medium text-slate-900 dark:text-white">Vendor</label>
            <select id="vendor" name="vendor" class="block p-2.5 w-full text-sm text-slate-900 bg-slate-50 rounded-lg border border-slate-300 focus:ring-slate-50 focus:border-slate-500 dark:bg-slate-800 dark:border-slate-600 dark:placeholder-slate-400 dark:text-white dark:focus:ring-slate-500 dark:focus:border-slate-500">
        ...
        <div class="mt-3">
            <label for="cost" class="block mb-2 text-lg font-medium text-slate-900 dark:text-white">Cost</label>
            <input type="number" name="cost"  id="cost" class="block p-2.5 w-full text-sm text-slate-900 bg-slate-50 rounded-lg border border-slate-300 focus:ring-slate-50 focus:border-slate-500 dark:bg-slate-800 dark:border-slate-600 dark:placeholder-slate-400 dark:text-white dark:focus:ring-slate-500 dark:focus:border-slate-500">
        </div>
                    ...
                    <div id="contentUpload" class="justify-center pt-5 pb-6">
                        ...
                        <div id="fileON" class=" gap-2 hidden">
                            <div id="cardimg1" class="flex flex-col gap-2 p-2 items-center">
                                <div id="box1" class="border-4 border-slate-400 text-lg text-slate-900 dark:text-slate-200 rounded-md dark:bg-slate-700">img
                                </div>
                                <p class="text-slate-900 dark:text-slate-200 whitespace-nowrap max-w-[10ch] truncate"> file1.png </p>
                            </div>
                        </div>
                        
                        <!-- Default state (no file selected) -->
                        <div id="fileOFF" class="flex flex-col items-center">
                            <svg class="w-8 h-8 mb-4 text-slate-500 dark:text-slate-400" fill="currentColor" xmlns="http://www.w3.org/2000/svg"
                            ...
                            <p class="mb-2 text-sm text-slate-500 dark:text-slate-400"><span class="font-semibold">Click to upload</span> or
                                drag and drop</p>
        ...
        <div class="mt-3">
            <label for="statusResolve" class="block text-lg font-medium text-slate-900 dark:text-white">Repair Result</label>
            <ul class="mt-2 grid lg:w-2/4 gap-4 md:grid-cols-2">
                <li>
                    <input type="radio" id="Finish" name="statusResolve" value="Finish" class="hidden peer"/>
                    <label id="statusResolve" for="Finish" class="inline-flex items-center justify-between w-full p-3 text-slate-300 bg-slate-50 border border-slate-200 rounded-lg cursor-pointer peer-checked:bg-slate-200 peer-checked:text-slate-900  peer-checked:shadow-md
                    dark:peer-checked:bg-slate-800 dark:peer-checked:text-white dark:peer-checked:border-slate-800
                    hover:text-slate-600 hover:bg-slate-100 dark:text-slate-400 dark:bg-slate-700 dark:hover:bg-slate-700 dark:border-0">                           
                        <div class="block">
                ...
                <li>
                    ...
                    <label id="statusResolve" for="Damaged" class="inline-flex items-center justify-between w-full p-3 text-slate-300 bg-slate-50 border border-slate-200 rounded-lg cursor-pointer peer-checked:bg-slate-200 peer-checked:text-slate-900  peer-checked:shadow-md
                    dark:peer-checked:bg-slate-800 dark:peer-checked:text-white dark:peer-checked:border-slate-800
                    hover:text-slate-600 hover:bg-slate-100 dark:text-slate-400 dark:bg-slate-700 dark:hover:bg-slate-700 dark:border-0">
                        <div class="block">
    ...
    @if ($imagesFile != null)
        ...
        <div id="default-modal-11" data-modal-id="11" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] ">
            <div class="relative p-4 w-full max-w-[85%] mx-auto max-h-full">
                <!-- Modal content -->
                <div class="relative w-full bg-white rounded-lg shadow-sm dark:bg-slate-800">
                    <!-- Modal header -->
                    <div class="flex items-center justify-between py-2 px-3 md:px-5 border-b rounded-t dark:border-slate-600 border-slate-200">
                        <h3 class="text-xl font-semibold text-slate-900 dark:text-white">
                            Image Documents
                        </h3>
                        <button type="button" class="text-slate-400 bg-transparent hover:bg-slate-200 hover:text-slate-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-slate-600 dark:hover:text-white" data-modal-hide="default-modal-11">
    ...
    <script>
        ...
        function validateForm() {
            ...
            else if (codeMaintence.length != 15) errors.push('Kode Maintenance tidak valid');
            if (!repairDetail) errors.push('Repair Detail tidak boleh kosong');
            else if (repairDetail.length > 300) errors.push('Detail Repair maksimal 300 karakter');
            if (!vendor) errors.push('Vendor tidak boleh kosong');
            if (!cost) errors.push('Cost tidak boleh kosong');
            else if (!/^\d+$/.test(cost)) errors.push('Cost hanya boleh berisi angka');
            ...
            if (fileReport.length > 0) {
                ...
                for (let file of fileReport) {
                    ...
                    if (!allowedExtensions.includes(fileExt)) errors.push(`Format file "${file.name}" harus berupa PDF, PNG, JPG, atau JPEG`);
                    if (fileSize > 2 * 1024 * 1024) errors.push(`Ukuran file "${file.name}" terlalu besar (maks. 2 MB)`);
                }
            }
            return errors;
        }
                ...
                if (files.length > 0) {
                    ...
                    Array.from(files).forEach((file, index) => {
                        ...
                        reader.onload = function (e) {
                            ...
                            card.innerHTML = `
                                <div class="border-4 border-slate-400 text-lg text-slate-900 dark:text-slate-200 rounded-md  dark:bg-slate-700">
                                    <img src="${e.target.result}" class="w-16 h-20 object-cover"/>
                                </div>
                                <p class="text-slate-900 dark:text-slate-200 whitespace-nowrap max-w-[10ch] truncate">${file.name}</p>
                            `;
