    ...
    <div class="flex justify-between">
        ...
        <div class="flex items-center space-x-2">
            @can('edit-master-asset')
                <button 
                id="updateMasterAssetButton" 
                data-modal-target="updateMasterAssetModal" 
                data-modal-toggle="updateMasterAssetModal"  
                class="px-5 sm:px-10 py-2 text-center shadow-md bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 dark:text-white rounded-md">EDIT</button>
            @endcan
            @can('printqr-master-asset')
                <a href="{{ route('print', $assetMaster->slug ) }}" target="blank" >
                    <button class="px-5 truncate sm:px-10 py-2 text-center shadow-md bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 dark:text-white rounded-md">Print QR</button>
                </a>
            @endcan
        </div>
    </div>
    ...
    <div class="container mx-auto w-full mt-5">
        <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3 items-start">
            ...
            <div class="order-1 md:order-1 lg:order-1">
                ...
                <div class="flex w-full text-lg text-slate-600 dark:text-slate-200 space-x-1">
                    <div class="">
                        ...
                        <p class="truncate">Stock Currrent </p>
                        <p class="truncate">Category </p>
                    </div>
                    <div class="">
                        <p class="truncate">: {{ $assetMaster->interval_maintence ?? '-' }} M</p>
            ...
            <div class="relative order-3 md:col-span-2 md:order-3 lg:order-3 lg:col-span-1 ">
                <div class="relative h-60 bg-slate-200 flex rounded-md overflow-hidden">
        ...
        <div class="w-full flex justify-between mb-2">
            <div class="">
                ...
                <div class="w-20 relative">
                    <div class="absolute inset-y-0 rtl:inset-r-0 start-0 flex items-center ps-3 pointer-events-none">
                        <svg class="w-4 h-4 text-slate-400 dark:text-slate-400" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 20">
            ...
            @can('checkin')
                <a href="{{ route('showCheckIn', $assetMaster->slug) }}">
                    <button class="px-5 sm:px-10 py-2 text-center shadow-md bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 dark:text-white rounded-md">Add Asset</button>
                </a>
            @endcan
        </div>
                ...
                <tbody id="tableBody">
                    @forelse ($dataItemByAms as $item)
                        <tr class="text-slate-800 dark:text-slate-200 border-b border-slate-200 dark:border-slate-700 rounded-md">
                            ...
                            <td class="py-3 px-1 whitespace-nowrap max-w-[20ch] truncate">{{ $item->code_assets }}</td>
                            <td class="py-3 px-1 whitespace-nowrap max-w-[20ch] truncate">{{ $item->location->location_name ?? 'Unknown' }}</td>
                            <td class="py-3 px-1 whitespace-nowrap max-w-[20ch] truncate">{{ $item->condition ?? 'N/A' }}</td>
                            ...
                            @can('detail-item-asset')
                            <td class="py-3 px-1 whitespace-nowrap max-w-[20ch] truncate text-center">
                                <a href="{{ route('itemAsset', $item->code_assets ) }}" class="py-1.5 px-4 text-center shadow-md bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 dark:text-white rounded-md edit-button">
                                    Detail
                                </a>
                            </td>
                            @endcan
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="text-center py-3 text-slate-500 dark:text-slate-400">
                                not found master asset
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        <div class="relative p-4 w-full max-w-2xl h-full md:h-auto">
            ...
            <div class="relative p-4 bg-white rounded-lg shadow dark:bg-slate-800 sm:p-5">
                ...
                <div class="flex justify-between items-center pb-3 mb-3 border-b rounded-t sm:mb-3 dark:border-slate-600">
                    ...
                    <button type="button" class="text-slate-400 bg-transparent hover:bg-slate-200 hover:text-slate-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-slate-600 dark:hover:text-white" data-modal-toggle="updateMasterAssetModal">
                        <svg aria-hidden="true" class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path></svg>
                        <span class="sr-only">Close modal</span>
                    </button>
                </div>
                <!-- Modal body -->
                <div class="text-slate-900 dark:text-white mb-5">
                    <h1 class="text-3xl font-semibold">{{ $assetMaster->asset_name }}</h1>
                </div>
                <form id="editMasterAsset" action="{{ route('masterAsset.edit.action') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="grid gap-4 mb-4 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <input type="text" name="nameAsset" id="nameAsset" value="{{ $assetMaster->asset_name }}" class="bg-slate-50 border border-slate-300 text-slate-900 text-sm rounded-lg focus:ring-slate-50 focus:border-slate-600 block w-full p-2.5 dark:bg-slate-700 dark:border-slate-600 dark:placeholder-slate-400 dark:text-white dark:focus:ring-slate-500 dark:focus:border-slate-500" placeholder="name asset">
                        </div>
                        <div>
                            <label for="category" class="block mb-2 text-sm font-medium text-slate-900 dark:text-white">Category</label>
                            <select id="category" name="category" class="bg-slate-50 border border-slate-300 text-slate-900 text-sm rounded-lg focus:ring-slate-50 focus:border-slate-600 block w-full p-2.5 dark:bg-slate-700 dark:border-slate-600 dark:placeholder-slate-400 dark:text-white dark:focus:ring-slate-500 dark:focus:border-slate-500">
                        ...
                        <div>
                            <label for="maintenInterval" class="block mb-2 text-sm font-medium text-slate-900 dark:text-white">Mainten Interval</label>
                            <input type="text" name="maintenInterval" id="maintenInterval" value="{{ $assetMaster->interval_maintence }}" class="bg-slate-50 border border-slate-300 text-slate-900 text-sm rounded-lg focus:ring-slate-50 focus:border-slate-600 block w-full p-2.5 dark:bg-slate-700 dark:border-slate-600 dark:placeholder-slate-400 dark:text-white dark:focus:ring-slate-500 dark:focus:border-slate-500" placeholder="Mont">
                        </div>
                        <div>
                            <label for="stockMinimum" class="block mb-2 text-sm font-medium text-slate-900 dark:text-white">Minimum Stock</label>
                            <input type="text" name="stockMinimum" id="stockMinimum" value="{{ $assetMaster->min_stock }}" class="bg-slate-50 border border-slate-300 text-slate-900 text-sm rounded-lg focus:ring-slate-50 focus:border-slate-600 block w-full p-2.5 dark:bg-slate-700 dark:border-slate-600 dark:placeholder-slate-400 dark:text-white dark:focus:ring-slate-500 dark:focus:border-slate-500" placeholder="Mont">
                        </div>
                        <div class="sm:col-span-2">
                            <label class="block mb-2 text-sm font-medium text-slate-900 dark:text-white" for="fileImg">Upload Image Asset</label>
                            <input id="fileImg" name="fileImg" class="bg-slate-50 border border-slate-300 text-slate-900 text-sm rounded-lg focus:ring-slate-50 focus:border-slate-600 block w-full dark:bg-slate-700 dark:border-slate-600 dark:placeholder-slate-400 dark:text-white dark:focus:ring-slate-500 dark:focus:border-slate-500" aria-describedby="fileImg_help" id="fileImg" type="file">
                            <p class="mt-1 text-sm text-slate-500 dark:text-slate-300" id="fileImg_help">SVG, PNG, JPG (MAX. 800x400px).</p>
                        </div>
                        <div class="sm:col-span-2">
                            <label for="description" class="block mb-2 text-sm font-medium text-slate-900 dark:text-white">Description</label>
                            <textarea id="description" name="description" rows="5" class="bg-slate-50 border border-slate-300 text-slate-900 text-sm rounded-lg focus:ring-slate-50 focus:border-slate-600 block w-full p-2.5 dark:bg-slate-700 dark:border-slate-600 dark:placeholder-slate-400 dark:text-white dark:focus:ring-slate-500 dark:focus:border-slate-500" placeholder="Write a description...">{{ $assetMaster->description }}</textarea>                    
                        </div>
                    </div>
        ...
        <div class="relative p-4 w-full max-w-[85%] max-h-full">
            ...
            <div class="relative bg-white rounded-lg shadow-sm dark:bg-slate-700">
                ...
                <div class="flex items-center justify-between p-3 md:p-4 border-b rounded-t dark:border-slate-600 border-slate-200">
                    ...
                    <button type="button" class="text-slate-400 bg-transparent hover:bg-slate-200 hover:text-slate-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-slate-600 dark:hover:text-white" data-modal-hide="modal_imgMasterAsset">
                ...
                <div class="flex items-center p-3 md:p-4 border-t border-slate-200 rounded-b dark:border-slate-600">
                    <button data-modal-hide="modal_imgMasterAsset" type="button" class="text-white bg-slate-700 hover:bg-slate-800 focus:ring-4 focus:outline-none focus:ring-slate-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-slate-600 dark:hover:bg-slate-700 dark:focus:ring-slate-800">Close</button>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    function validateForm() {
        ...
        if (nameAsset === '') errors.push('Name Asset tidak boleh kosong');
        if (category === '') errors.push('Category tidak boleh kosong');
        if (maintenInterval === '') errors.push('Mainten Interval tidak boleh kosong');
        else if (!/^\d+$/.test(maintenInterval)) errors.push('Mainten Interval hanya boleh berisi angka');
        if (stockMinimum === '') errors.push('Stock Minimum tidak boleh kosong');
        else if (!/^\d+$/.test(stockMinimum)) errors.push('Stock Minimum hanya boleh berisi angka');
        ...
        if (fileImg.length > 0) {
            ...
            if (!allowedExtensions.includes(fileExt)) errors.push('Format file harus berupa PDF, PNG, JPG, atau JPEG');
            if (fileSize > 2 * 1024 * 1024) errors.push('Ukuran file terlalu besar (maks. 2 MB)');
        }
        return errors;
    }

    // Event listener untuk form submission
    document.getElementById('editMasterAsset').addEventListener('submit', function (e) {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        noResultsMessage.innerHTML = '<td colspan="7" class="text-center py-3 text-slate-500 dark:text-slate-400">Data not found</td>';
